...
if [ -f "$config_file" ]; then
	...
	if grep -q "^\[keystone_authtoken\]" "$config_file"; then
	    ...
	    sudo sed -i "/^\[keystone_authtoken\]/a$keystone_authtoken_config" "$config_file"
	...
	if grep -q "^\[glance_store\]" "$config_file"; then
	    ...
	    # Add the paste_deploy below the [glance_store] section
	    sudo sed -i "/^\[glance_store\]/a$glance_store_conf" "$config_file"
	...
	if grep -q "^\[oslo_limit\]" "$config_file"; then
	    ...
	    # Add the paste_deploy below the [oslo_limit] section
	    sudo sed -i "/^\[oslo_limit\]/a$oslo_limit_conf" "$config_file"
	    echo "glance_store added to [oslo_limit] section"
